...
function wimbblock_exclude_init() {
	add_settings_section( 'wimbblock_exclude', '', '__return_empty_string', 'wimbblock_exclude' );
	...
	register_setting(
		...
		array(
			'type'              => 'array',
			'sanitize_callback' => 'wimbblock_exclude_validate',
			'default'           => array(),
		)
	);
}
...
function wimbblock_exclude_form() {
	$all = wimbblock_get_exclude();
	$i   = 0;
	foreach ( $all as $browser ) {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		echo '<p><input type="text" size="15" name="wimbblock_exclude[' . $i . ']" value="' . esc_html( $browser ) . '" /> ' . "\n";
		++$i;
	}
	if ( $i === 0 ) {
	...
	// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	echo '<p><input type="text" size="15" name="wimbblock_exclude[' . $i . ']" ' . wp_kses_post( $placeholder ) . ' /> ' . "\n";
}
