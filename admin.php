		...
		if ( is_multisite() && ! is_main_site() && is_plugin_active_for_network( WIMB_BASENAME ) ) {
			...
			echo wp_kses_post(
				wp_sprintf(
					...
					'<a href="' . get_site_url( get_main_site_id() ) . '/wp-admin/admin.php?page=' . WIMB_NAME . '">',
					'</a>'
				)
			);
			echo '</p>';
...
function wimbblock_admin_robots_tabs() {
	$tab        = filter_input(
		INPUT_GET,
		'tab',
		FILTER_CALLBACK,
	...
	$tabs[] = array(
		'tab'   => 'blocking',
		'title' => __( 'Blog browsers, bots and crawlers', 'wimb-and-block' ),
	);
	$tabs[] = array(
		'tab'   => 'table',
